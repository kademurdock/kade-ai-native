...
struct ConversationDetailView: View {
    ...
    var body: some View {
        ...
        .onChange(of: sendState) { old, new in
            if case .sending = new {
                ...
                speechWorkPhase = .finished
                speechGapResume?.cancel()
                speechGapResume = nil
                if spokenTurnStarted { Earcons.shared.stopWaitingLoop() }
    ...
    private func scheduleThinkingResume() {
        guard speechGapResume == nil,
              SpeechWaitPolicy.shouldResume(phase: speechWorkPhase, turnLive: spokenTurnLive,
                clipPlaying: voiceService.isClipPlaying, paused: voiceService.isPaused,
                speechQueued: voiceService.isSpeaking) else { return }
        speechGapResume = Task { @MainActor in
            try? await Task.sleep(nanoseconds: speechGapResumeDelay)
            guard !Task.isCancelled else { return }
            speechGapResume = nil
            guard case .sending = sendState,
                  SpeechWaitPolicy.shouldResume(phase: speechWorkPhase, turnLive: spokenTurnLive,
                    clipPlaying: voiceService.isClipPlaying, paused: voiceService.isPaused,
                    speechQueued: voiceService.isSpeaking) else { return }
    ...
    private func performSend(text: String, parentId: String?, includeAttachment: Bool = true) async {
        ...
        do {
            ...
            let resolvedConversationId = try await messageSendingService.send(
                ...
                onThink: { chunk in
                    if !chunk.isEmpty { speechWorkPhase = .thinking; scheduleThinkingResume() }
