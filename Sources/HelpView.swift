    ...
    static let all: [HelpSection] = [
        ...
        HelpSection(title: "What's new", entries: [
            ...
            HelpEntry(
                ...
                body: "For sighted family: the call screen now shows a soft breathing orb that changes color with the call — teal listening, amber thinking, green with ripples while your companion talks — replies wear a small colored initial circle so you can see who's speaking at a glance, and empty screens have gentle artwork. None of it changes what VoiceOver says, and it all goes still under Reduce Motion. Calls got a tune-up: greetings are short now — just who picked up, no more 'go ahead, I'm listening' — the typing sound finally works on app calls while your companion thinks, and calling from inside a conversation continues that conversation: same chat, same memory, one thread per session. Start a new conversation when you want a fresh call. Agents also now clearly know a text chat from a live call, and lean harder into emotional voice steering on texts. Also: the chat composer gained a paperclip: attach a photo or file to your next message and your companion actually looks at it — one attachment per message, with a spoken chip you can remove. Calls gained a Mute button between Stop Talking and Hang Up, so a cough or the TV can't interrupt your companion mid-sentence. Every conversation row gained Share: copy a public link anyone can read in a browser, stop sharing any time, or export the whole chat as a text file. And Settings grew Password & Account — change your password right in the app, or delete the account entirely, asked twice. Also: a visual and layout freshen-up, none of which changes what VoiceOver says: messages now sit in soft rounded bubbles, the home screen's tools are a compact two-column grid of tiles, the voice button moved from the chat's top bar down beside the name of who you're talking to, and the context gauge only appears once a conversation is actually filling up. Every reply now has a Reading view — flick to it in the Actions rotor on any reply, or use the actions menu — a full-screen, distraction-free page with big print in your chosen reading font, one paragraph per swipe, and a close button that puts you right back on the message. New chats finally get real names: the server now names every conversation you start here after your first exchange, the way it always did on the web — no more lists full of rows all called New Chat. Opening a chat right after making one is solid under VoiceOver too. Archived conversations got their own screen — the archive box button in your conversation list — where you can read them, restore them, or delete them for good. And every conversation gained a search button that narrows the messages to the ones whose text matches what you type."
            ),
            HelpEntry(
                title: "Earlier",
        ...
        HelpSection(title: "Getting around", entries: [
            HelpEntry(
                ...
                body: "Sign in once and the app remembers you. Everything is grouped in three sections you can jump between with the Headings rotor: Talk holds Call your Spotter, Your conversations, and Alerts. Tools holds Transcribe, Describe, Matchmaker, The Parlor, Kade's Clubhouse, Game Room, Debate Room, Agent Builder, My Creations, and the Wall of Fame. Settings and help holds Settings, this help, the full web app, and Sign out."
            ),
            HelpEntry(
        ...
        HelpSection(title: "Kade's Clubhouse", entries: [
            HelpEntry(
                title: "What it is",
                body: "Live voice rooms for the family — real stereo sound, person to person on Kade's own room server. Tap Kade's Clubhouse on the home screen, pick a room like The Porch, and you're in with your mic live. The roster says who's here and who's talking, one button mutes your mic, and another reads the room out loud. Joining can take a few extra seconds while a sleeping room server wakes up — the screen says so while it happens."
            ),
            HelpEntry(
            ...
            HelpEntry(
                ...
                body: "Passcode rooms so groups can group up. Open a room with a name and a speakable passcode — letters and numbers only — and share the code with your people; the room opens only with it. A Parlor party table's code works as a passcode too, so one code can carry both the cards and the voices. Whoever opened a room can close it for good."
            ),
            HelpEntry(
