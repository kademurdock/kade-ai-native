...
@main enum WorldModelsTests {
    static func main() throws {
        ...
        let senses = try decoder.decode(WorldPictureRoom.Senses.self, from: Data(#"{"ambience":"amb.woods.night"}"#.utf8))
        precondition(senses.ambience == "amb.woods.night" && senses.nature == nil)
        precondition(picture.sensory?.ambience == nil)
        var speech = WorldSpeechBuffer()
        precondition(speech.receiveLive("Pat arrives.") == "Pat arrives.")
        precondition(speech.beginReply() && !speech.beginReply())
        precondition(speech.receiveLive("The door opens.") == nil)
        precondition(speech.receiveLive("Mira says hello.") == nil)
        let fullReply = String(repeating: "The shop has shelves, clocks, and a west door. 🕰️ ", count: 200)
        precondition(speech.finishReply(fullReply) == fullReply + " The door opens. Mira says hello.")
        precondition(!speech.replying && speech.receiveLive("A bell.") == "A bell.")
        precondition(speech.beginReply())
        _ = speech.receiveLive("A visitor.")
        precondition(speech.finishReply("Network error.") == "Network error. A visitor.")
        precondition(speech.beginReply())
        _ = speech.receiveLive("Old room.")
        speech.clearLive()
        precondition(speech.finishReply("") == "")
        print("World models: 31 checks passed. Covers full reply/live-event ordering, decoding and cache identity; not VoiceOver playback, AVAudioPlayer or WebKit rendering.")
    }
}
